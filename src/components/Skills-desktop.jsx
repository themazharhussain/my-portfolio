...
const Skills = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (prefersReducedMotion) {
        ...
        gsap.set([titleRef.current, skillsRef.current?.children], { opacity: 1, y: 0 });
        return;
      }
      ...
      // Skills cards animation - simplified on mobile
  ...
  return (
    ...
    >
      {/* Honeycomb pattern background - reduced complexity on mobile */}
      {!isMobile && (
      ...
      {/* Layered gradient orbs - reduced on mobile */}
      <div className={`absolute top-0 left-1/4 ${isMobile ? 'w-[400px] h-[400px]' : 'w-[700px] h-[700px]'} bg-emerald-600 rounded-full mix-blend-screen filter blur-3xl ${isMobile ? 'opacity-10' : 'opacity-20'} animate-float-up`}></div>
      {!isMobile && <div className="absolute bottom-0 right-1/4 w-[700px] h-[700px] bg-cyan-500 rounded-full mix-blend-screen filter blur-3xl opacity-15 animate-float-up" style={{ animationDelay: '3s' }}></div>}
      {!isMobile && <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-teal-500 rounded-full mix-blend-screen filter blur-3xl opacity-10 animate-float-up" style={{ animationDelay: '1.5s' }}></div>}

      {/* Cross-hatch pattern - desktop only */}
      {!isMobile && (
        <div className="absolute inset-0 opacity-5">
